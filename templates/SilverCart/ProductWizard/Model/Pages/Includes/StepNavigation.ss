...
<div class="row mb-10 text-center">
    ...
    <div class="col-4 text-blue-dark">
        <a href="{$Link}">
            <% if $FontAwesomeIcon %>
            <span class="fa fa-2x fa-{$FontAwesomeIcon} border border-blue-dark rounded-circle p-10"></span>
            <% end_if %>
            <span class="h1 p-relative l-8 b-2">{$Pos}. {$Title}</span>
            <span class="fa fa-2x fa-check p-10"></span>
        </a>
    </div>
        <% else_if $IsCurrent %>
    ...
    <div class="col-4 text-gray">
            <% if $FontAwesomeIcon %>
        <span class="fa fa-2x fa-{$FontAwesomeIcon} border border-gray rounded-circle p-10"></span>
            <% end_if %>
        <span class="h1 p-relative l-8 b-2">{$Pos}. {$Title}</span>
    </div>
        <% end_if %>
